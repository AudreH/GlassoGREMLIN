...
hc = hclust(dist(Sigma2), method = "complete")
ComplexHeatmap::draw(Heatmap(Sigma_sim,
                             top_annotation = column_ha, name = "CovData", column_title = "Simulated Covariance structure",
                             row_dend_reorder = hc$order, column_dend_reorder = hc$order,
                             cluster_rows = hc, cluster_columns = hc,
...
ComplexHeatmap::draw(Heatmap(w, top_annotation = column_ha2, name = "Cov estimation", column_title = "gl1$w ordered by group from GREMLIN",
                             col = colorRamp2(c(min(wi2), 0, max(wi2)), c("blue", "white", "red")),
                             row_order = order(clustr), column_order = order(clustr),
                             right_annotation = row_ha,
...
```{r}
n_iter = 10
lambda_seq = c(10^-5, 10^-4, 0.0005, 10^-3)
```

```{r}
listLambda = loopGLGR(lambda_seq, dat, Omega_sim, n_iter)
```
## Graphes et sorties
...
```{r}
listLambda = loopGLBM(lambda_seq, dat, Omega_sim, n_iter)
```
## Graphes et sorties
...
df_test = do.call("rbind",lapply(1:length(listLambda), FUN = function(i){
  ...
  df = data.frame(t(do.call("cbind", list("Clusters" = c(NA, unlist(liste$clusters)), "NID" = c(NA, unlist(liste$NID)),
                                 "diff_sim" = c(unlist(liste$diff_sim)), "diff_prev_iter" = c(NA, unlist(liste$diff_prev_iter))))))
  colnames(df) = c("GL0", "BMGL0", paste0("BMGL", 1:(ncol(df)-2)))
  df$response = rownames(df)
  df$lambda = lambda_seq[i] 
  df
}))
