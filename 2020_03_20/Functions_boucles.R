...
loopGLGR = function(lambda_seq, dat = NULL, Sigma = NULL, Omega_sim, n_iter, classification, threshold =10^-6,
  ...
  listLambda = mclapply(lambda_seq, FUN = function(lam){
    ...
    gl0 = glassoFast(S = Sigma, rho = matrix(lam, ncol = ncol(Sigma), nrow = nrow(Sigma)))
    ...
    if(!is.null(gr0)){
      ...
      gl1 = glassoFast(S = Sigma, rho = pmax(matrix(10^-5, ncol(link), ncol(link)), lam*(1-link)),
                       start = "warm", wi.init = gl0$wi, w.init = gl0$w)
      ...
      if(!any(is.na(gl1$wi))){
        ...
        while(list_diff_iter_prec[[length(list_diff_iter_prec)]]>=threshold && i<n_iter){
          ...
          if(!is.null(gr)){ # convergence
            ...
            gl = glassoFast(S = Sigma, rho = pmax(matrix(10^-5, ncol(link), ncol(link)), lam*(1-link)), start = "warm", wi.init = list_res_glasso[[length(list_res_glasso)]]$wi, w.init = list_res_glasso[[length(list_res_glasso)]]$w)
        ...
        paste0("Exited from algorithm before the loop - GREMLIN did not converge")
      }# ENDIF any gl1$wi == na
    }# ENDIF gr0 == NULL
