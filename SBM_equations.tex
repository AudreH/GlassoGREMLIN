...
\paragraph{Graphical Lasso}
Originally,  to estimate $\Omega$, it is standard to consider the lasso penalization
\begin{align*}
\mathcal{S}(X,\Omega,\lambda) &=  \log \mathbb{P}(X | \Omega) - \lambda^{-1}  \|\Omega\|_1\\
&=   \log \mathbb{P}(X | \Omega) - \lambda^{-1} \sum_{i < j} |\Omega_{ij}| \\
...
&=  \log \mathbb{P}(\Omega;\lambda) + Cste + \frac{p(p-1)}{2} \log(\lambda) 
\end{align*}
where $ \mathbb{P}(\Omega;\lambda) $ is such that the $\Omega_{ij} \sim_{i.i.d} Laplace(0,\lambda)$, $i< j$.   As a consequence : 
\begin{align*}
\widehat \Omega  &= \arg \max_{\Omega} \mathcal{S}(X,\Omega,\lambda) \\
\mathcal{S}(X,\Omega,\lambda) &=  \log \mathbb{P}(X | \Omega) +  \log \mathbb{P}(\Omega;\lambda)  + \frac{p(p-1)}{2} \log(\lambda) \\
&= \log \mathbb{P}(X | \Omega) +  \log \left[\mathbb{P}(\Omega;\lambda)  N(\lambda)  \right]
\end{align*}
where  $N(\lambda)$ is the normalizing constant of $\mathbb{P}(\Omega;\lambda)$. 
...
\item If we solve : 
\begin{align*}
\widehat (\Omega, \widehat \lambda)  &= \arg \max_{\Omega,\lambda} \widetilde{\mathcal{S}}(X,\Omega,\lambda) \\
 \widetilde{\mathcal{S}}(X,\Omega,\lambda)  &=  \log \mathbb{P}(X | \Omega) +  \log \mathbb{P}(\Omega;\lambda)  \\
 &=  \mathcal{S}(X,\Omega,\lambda)  -   \frac{p(p-1)}{2} \log(\lambda)
\end{align*}
we are doing something else. Does it have a sense to do that? 
\end{itemize}


\paragraph{Graphical SBM with $K$ fixed (number of clusters)}
\begin{align*}
\widehat \Omega  &= \arg \max_{\Omega} \mathcal{S}(X,\Omega) \\
\mathcal{S}(X,\Omega) &=  \log \mathbb{P}(X | \Omega) +  \log \mathbb{P}_{SBM(K)}(\Omega; \theta)    + \log N_{SBM(K)}(\theta)
\end{align*}

 $\log \mathbb{P}_{SBM(K)}(\Omega; \theta)$ is the normalizing constant. 
